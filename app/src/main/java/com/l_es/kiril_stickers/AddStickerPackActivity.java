...
public abstract class AddStickerPackActivity extends BaseActivity {
    ...
    private Intent createIntentToAddStickerPack(String identifier, String stickerPackName) {
        ...
        intent.putExtra(Utilities.EXTRA_STICKER_PACK_ID, identifier);
        intent.putExtra(Utilities.EXTRA_STICKER_PACK_AUTHORITY, BuildConfig.CONTENT_PROVIDER_AUTHORITY);
        intent.putExtra(Utilities.EXTRA_STICKER_PACK_NAME, stickerPackName);
        return intent;
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ADD_PACK) {
            ...
            }else if(resultCode == Activity.RESULT_OK && !sent){
                // Add new downloads to statistics server
                DocumentReference documentReference = collectionReference.document(packName);
                documentReference.get().addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        documentReference.update(Utilities.DOWNLOADS, FieldValue.increment(1));
                        sent = true;
                    }
                });
            }
